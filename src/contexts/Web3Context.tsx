...
import { fetchPaintswapNFTs } from '@/lib/web3/paintswap';
...
export const Web3Provider = ({ children }: { children: ReactNode }) => {
  ...
  const [paintswapNFTs, setPaintswapNFTs] = useState<NFTMetadata[]>([]);
  ...
  const refreshNFTs = async () => {
    ...
    try {
      const [onChainNFTs, paintswapNFTs] = await Promise.all([
        fetchOwnedNFTs(address),
        fetchPaintswapNFTs(address),
      ]);

      setSonicNFTs(onChainNFTs);
      setTinHatCatters(onChainNFTs);

      setPaintswapNFTs(paintswapNFTs);
      console.log("PaintSwap NFTs:", paintswapNFTs);
      // Optionally: setPaintswapNFTs(paintswapNFTs); — if you want to display or filter elsewhere
    } catch (err) {
